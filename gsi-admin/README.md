...
**이 폴더 하나가 최종본이다.** `v1-approved/`(KT 통과 1차 시안)를 출발점으로 KT 디자인 시스템에 맞춰 정렬한 결과물이며, 바깥 폴더에 의존하지 않는다 — 통째로 복사해 아무 정적 서버(또는 파일 더블클릭)로 열린다. KT 피그마 원본(`GSI_admin.fig` Component_01~04 · 편성표 1:23894)에 맞춰 정렬 중이며, 근거·미적용 항목은 `../기능정의/DS-MATCH-260819.md`.

## 인계 문서
기능 명세 · 더미 ↔ 실데이터 접합 지점 · 보류 기능 · KT 확인 필요 항목은 **`../기능정의/HANDOFF.md`** 에 있다.
